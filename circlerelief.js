// Stochastic Elements — "craters / holes" embossed look
// Cliquez pour ajouter un burst. Touche R pour reset.

let bg;
let countPerFrame = 6;   // dépôts par frame
let maxTries = 30;       // anti-overlap (simple)
let circles = [];        // mémorise les cratères pour éviter trop de chevauchement

function setup() {
  createCanvas(520, 340);
  pixelDensity(2);
  bg = color(170); // fond gris
  background(bg);
  noFill();
}

function draw() {
  // Dépose quelques éléments par frame
  for (let i = 0; i < countPerFrame; i++) {
    trySpawnCrater();
  }

  // Stop quand il y en a "assez"
...
function trySpawnCrater() {
  for (let t = 0; t < maxTries; t++) {
    ...
    // anti-overlap léger (pas parfait mais aide)
    if (!tooClose(x, y, r)) {
      circles.push({ x, y, r });
      drawCrater(x, y, r);
      break;
...
function tooClose(x, y, r) {
  // check rapide sur un petit échantillon récent
...
function drawCrater(x, y, r) {
  // Direction lumière : top-left
  let dx = -0.18 * r;
  let dy = -0.18 * r;

  // Corps avec dégradé radial (donne le côté "pocket")
  let ctx = drawingContext;
  let g = ctx.createRadialGradient(
    x + dx, y + dy, r * 0.05,
    x, y, r
  );
  // légèrement plus clair côté lumière, plus sombre au centre/bord
  ...
  // Rim emboss: highlight (haut-gauche) + shadow (bas-droite)
  strokeWeight(max(1, r * 0.10));
  noFill();

  // Highlight arc (environ 220° -> 40°)
  stroke(230, 230, 230, 170);
  arc(x, y, 2 * r, 2 * r, radians(215), radians(35));

  // Shadow arc (environ 35° -> 215°)
  stroke(70, 70, 70, 140);
  arc(x, y, 2 * r, 2 * r, radians(35), radians(215));

  // Micro relief décalé (petit liseré secondaire)
